...
class XMLFileFormat(BaseFileFormat):
    ...
    def load(cls, *args: Any, source_file: str = '', sort_transactions: bool = True, **kwargs: Any) -> GnuCashFile:
        ...
        books: List[ElementTree.Element] = root.findall('gnc:book', cls.XML_NAMESPACES)
        for book in books:
    ...
    def create_book_from_xml(cls, book_node: ElementTree.Element, sort_transactions: bool = True) -> Book:
        ...
        """
        new_book = Book()
        book_id_node: Optional[ElementTree.Element] = book_node.find('book:id', cls.XML_NAMESPACES)
        if book_id_node is not None and book_id_node.text:
            new_book.guid = book_id_node.text
        accounts: List[ElementTree.Element] = book_node.findall('gnc:account', cls.XML_NAMESPACES)
        transactions: List[ElementTree.Element] = book_node.findall('gnc:transaction', cls.XML_NAMESPACES)
        slots: Optional[ElementTree.Element] = book_node.find('book:slots', cls.XML_NAMESPACES)

        if slots is not None:
            for slot in slots.findall('slot'):
                new_book.slots.append(cls.create_slot_from_xml(slot))

        commodities: List[ElementTree.Element] = book_node.findall('gnc:commodity', cls.XML_NAMESPACES)
        ...
        budgets: Optional[List[ElementTree.Element]] = book_node.findall('gnc:budget', cls.XML_NAMESPACES)
    ...
    def create_slot_from_xml(cls, slot_node: ElementTree.Element) -> Slot:
        """
         ...
         :rtype: Slot
         """
        key_node: Optional[ElementTree.Element] = slot_node.find('slot:key', cls.XML_NAMESPACES)
        if key_node is None or not key_node.text:
            raise ValueError('slot:key missing or empty in slot node')
        key: str = key_node.text
        value_node: Optional[ElementTree.Element] = slot_node.find('slot:value', cls.XML_NAMESPACES)
    ...
    def create_commodity_from_xml(cls, commodity_node: ElementTree.Element) -> Commodity:
        ...
        :rtype: Commodity
        """
        commodity_id_node: Optional[ElementTree.Element] = commodity_node.find('cmdty:id', cls.XML_NAMESPACES)
        ...
        commodity_space_node: Optional[ElementTree.Element] = commodity_node.find('cmdty:space', cls.XML_NAMESPACES)
        ...
        if commodity_node.find('cmdty:get_quotes', cls.XML_NAMESPACES) is not None:
            new_commodity.get_quotes = True

        quote_source_node = commodity_node.find('cmdty:quote_source', cls.XML_NAMESPACES)
        if quote_source_node is not None:
            new_commodity.quote_source = quote_source_node.text

        if commodity_node.find('quote_tz', cls.XML_NAMESPACES) is not None:
            new_commodity.quote_tz = True

        name_node: Optional[ElementTree.Element] = commodity_node.find('cmdty:name', cls.XML_NAMESPACES)
        if name_node is not None:
            new_commodity.name = name_node.text

        xcode_node: Optional[ElementTree.Element] = commodity_node.find('cmdty:xcode', cls.XML_NAMESPACES)
        if xcode_node is not None:
            new_commodity.xcode = xcode_node.text

        fraction_node: Optional[ElementTree.Element] = commodity_node.find('cmdty:fraction', cls.XML_NAMESPACES)
    ...
    def create_account_from_xml(cls, account_node: ElementTree.Element, account_objects: List[Account]) -> Account:
        ...
        account_guid_node = account_node.find('act:id', cls.XML_NAMESPACES)
        ...
        account_name_node = account_node.find('act:name', cls.XML_NAMESPACES)
        if account_name_node is not None and account_name_node.text:
            account_object.name = account_name_node.text
        account_type_node = account_node.find('act:type', cls.XML_NAMESPACES)
        if account_type_node is not None and account_type_node.text:
            account_object.type = account_type_node.text

        commodity: Optional[ElementTree.Element] = account_node.find('act:commodity', cls.XML_NAMESPACES)
        if commodity is not None and commodity.find('cmdty:id', cls.XML_NAMESPACES) is not None:
            account_object.commodity = cls.create_commodity_from_xml(commodity)
        else:
            account_object.commodity = None

        commodity_scu: Optional[ElementTree.Element] = account_node.find('act:commodity-scu', cls.XML_NAMESPACES)
        if commodity_scu is not None:
            account_object.commodity_scu = commodity_scu.text

        slots: Optional[ElementTree.Element] = account_node.find('act:slots', cls.XML_NAMESPACES)
        if slots is not None:
            for slot in slots.findall('slot', cls.XML_NAMESPACES):
                account_object.slots.append(cls.create_slot_from_xml(slot))

        code: Optional[ElementTree.Element] = account_node.find('act:code', cls.XML_NAMESPACES)
        if code is not None:
            account_object.code = code.text

        description: Optional[ElementTree.Element] = account_node.find('act:description', cls.XML_NAMESPACES)
        if description is not None:
            account_object.description = description.text

        parent: Optional[ElementTree.Element] = account_node.find('act:parent', cls.XML_NAMESPACES)
    ...
    def create_transaction_from_xml(cls, transaction_node: ElementTree.Element,
        ...
        guid_node: Optional[ElementTree.Element] = transaction_node.find('trn:id', cls.XML_NAMESPACES)
        if guid_node is not None and guid_node.text:
            transaction.guid = guid_node.text
        date_entered_node: Optional[ElementTree.Element] = transaction_node.find('trn:date-entered', cls.XML_NAMESPACES)
        if date_entered_node is not None:
            date_entered_ts_node: Optional[ElementTree.Element] = date_entered_node.find('ts:date', cls.XML_NAMESPACES)
        ...
        date_posted_node: Optional[ElementTree.Element] = transaction_node.find('trn:date-posted', cls.XML_NAMESPACES)
        if date_posted_node is not None:
            date_posted_ts_node: Optional[ElementTree.Element] = date_posted_node.find('ts:date', cls.XML_NAMESPACES)
        ...
        description_node: Optional[ElementTree.Element] = transaction_node.find('trn:description', cls.XML_NAMESPACES)
        if description_node is not None and description_node.text:
            transaction.description = description_node.text

        memo: Optional[ElementTree.Element] = transaction_node.find('trn:num', cls.XML_NAMESPACES)
        if memo is not None:
            transaction.memo = memo.text

        currency_node = transaction_node.find('trn:currency', cls.XML_NAMESPACES)
        if currency_node is not None:
            currency_id_node = currency_node.find('cmdty:id', cls.XML_NAMESPACES)
            currency_space_node = currency_node.find('cmdty:space', cls.XML_NAMESPACES)
        ...
        slots: Optional[ElementTree.Element] = transaction_node.find('trn:slots', cls.XML_NAMESPACES)
        if slots:
            for slot in slots.findall('slot', cls.XML_NAMESPACES):
                transaction.slots.append(cls.create_slot_from_xml(slot))

        splits: Optional[ElementTree.Element] = transaction_node.find('trn:splits', cls.XML_NAMESPACES)
    ...
    def create_split_from_xml(cls, split_node: ElementTree.Element, account_objects: List[Account]) -> Split:
        ...
        :rtype: Split
        """
        account_node: Optional[ElementTree.Element] = split_node.find('split:account', cls.XML_NAMESPACES)
        ...
        value_node: Optional[ElementTree.Element] = split_node.find('split:value', cls.XML_NAMESPACES)
        ...
        guid_node = split_node.find('split:id', cls.XML_NAMESPACES)
        if guid_node is not None and guid_node.text:
            new_split.guid = guid_node.text

        split_memo: Optional[ElementTree.Element] = split_node.find('split:memo', cls.XML_NAMESPACES)
        if split_memo is not None:
            new_split.memo = split_memo.text

        split_action: Optional[ElementTree.Element] = split_node.find('split:action', cls.XML_NAMESPACES)
        if split_action is not None:
            new_split.action = split_action.text

        quantity_node = split_node.find('split:quantity', cls.XML_NAMESPACES)
    ...
    def create_scheduled_transaction_from_xml(cls, xml_obj: ElementTree.Element,
        ...
        sx_transaction_guid: Optional[str] = cls.read_xml_child_text(xml_obj, 'sx:id', cls.XML_NAMESPACES)
        if sx_transaction_guid is not None and sx_transaction_guid:
            new_obj.guid = sx_transaction_guid
        new_obj.name = cls.read_xml_child_text(xml_obj, 'sx:name', cls.XML_NAMESPACES)
        new_obj.enabled = cls.read_xml_child_boolean(xml_obj, 'sx:enabled', cls.XML_NAMESPACES)
        new_obj.auto_create = cls.read_xml_child_boolean(xml_obj, 'sx:autoCreate', cls.XML_NAMESPACES)
        new_obj.auto_create_notify = cls.read_xml_child_boolean(xml_obj, 'sx:autoCreateNotify', cls.XML_NAMESPACES)
        new_obj.advance_create_days = cls.read_xml_child_int(xml_obj, 'sx:advanceCreateDays', cls.XML_NAMESPACES)
        new_obj.advance_remind_days = cls.read_xml_child_int(xml_obj, 'sx:advanceRemindDays', cls.XML_NAMESPACES)
        new_obj.instance_count = cls.read_xml_child_int(xml_obj, 'sx:instanceCount', cls.XML_NAMESPACES)
        new_obj.start_date = cls.read_xml_child_date(xml_obj, 'sx:start', cls.XML_NAMESPACES)
        new_obj.last_date = cls.read_xml_child_date(xml_obj, 'sx:last', cls.XML_NAMESPACES)
        new_obj.end_date = cls.read_xml_child_date(xml_obj, 'sx:end', cls.XML_NAMESPACES)

        template_account_node: Optional[ElementTree.Element] = xml_obj.find('sx:templ-acct', cls.XML_NAMESPACES)
        ...
        schedule_node: Optional[ElementTree.Element] = xml_obj.find('sx:schedule', cls.XML_NAMESPACES)
        if schedule_node is not None:
            recurrence_node = schedule_node.find('gnc:recurrence', cls.XML_NAMESPACES)
            if recurrence_node is not None:
                new_obj.recurrence_multiplier = cls.read_xml_child_int(
                    recurrence_node, 'recurrence:mult', cls.XML_NAMESPACES
                )
                new_obj.recurrence_period = cls.read_xml_child_text(
                    recurrence_node, 'recurrence:period_type', cls.XML_NAMESPACES)
                new_obj.recurrence_start = cls.read_xml_child_date(
                    recurrence_node, 'recurrence:start', cls.XML_NAMESPACES)
    ...
    def create_budget_from_xml(cls, budget_node: ElementTree.Element) -> Budget:
        ...
        id_node: Optional[ElementTree.Element] = budget_node.find('bgt:id', cls.XML_NAMESPACES)
        if id_node is not None and id_node.text:
            new_obj.guid = id_node.text

        name_node: Optional[ElementTree.Element] = budget_node.find('bgt:name', cls.XML_NAMESPACES)
        if name_node is not None:
            new_obj.name = name_node.text

        description_node: Optional[ElementTree.Element] = budget_node.find('bgt:description', cls.XML_NAMESPACES)
        if description_node is not None:
            new_obj.description = description_node.text

        period_count_node: Optional[ElementTree.Element] = budget_node.find('bgt:num-periods', cls.XML_NAMESPACES)
        if period_count_node is not None and period_count_node.text:
            new_obj.period_count = int(period_count_node.text)

        recurrence_node: Optional[ElementTree.Element] = budget_node.find('bgt:recurrence', cls.XML_NAMESPACES)
        if recurrence_node is not None:
            multiplier_node: Optional[ElementTree.Element] = recurrence_node.find('recurrence:mult', cls.XML_NAMESPACES)
            ...
            if recurrence_start_node is not None:
                gdate_node: Optional[ElementTree.Element] = recurrence_start_node.find('gdate', cls.XML_NAMESPACES)
                if gdate_node is not None and gdate_node.text:
                    new_obj.recurrence_start = datetime.strptime(gdate_node.text, '%Y-%m-%d')

        slots: Optional[ElementTree.Element] = budget_node.find('bgt:slots', cls.XML_NAMESPACES)
        if slots:
            for slot in slots.findall('slot', cls.XML_NAMESPACES):
